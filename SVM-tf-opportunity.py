...
#Scaling data obtained from SVD
...
for C in Cs:
    ...
    cls.fit(xtrain_svd_scl ,ytrain)
    scores.append(cls.score(xvalid_svd_scl,yvalid))
...
ax.set_xscale('log')
ax.set_title("SVM-Growth")    



clf = SVC(C=1, kernel='sigmoid', gamma=1, decision_function_shape='ovo') 
...
import matplotlib.pyplot as plt
plt.plot(fpr,tpr)
plt.title("SVM-opporunity")
#plt.title("SVM-opporunity-auc=%.4f"%(auc))
